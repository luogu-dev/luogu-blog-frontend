<template>
  <div id="article-comments animated fadeInDown" class="comments">
    ...
    <form v-if="uid" @submit.prevent="postComment">
      <textarea v-model="commentContent" class="comments-message" rows=6 placeholder="写下您的评论.."></textarea><br>
    ...
    <table border=0 class="comments-view"  v-if="ready">
        <tr v-for="comment in comments">
          ...
          <a
            ...
            target="_blank"
          >
            <img :src="BlogGlobals.picAddress + '/upload/usericon/' + comment.Author.UID + '.png'" style="width:42px;height:42px">
          </a></td>
            <td><p style="line-height:auto"><b><a :href="BlogGlobals.luoguAddress +'/space/show?uid=' + comment.Author.UID">{{ comment.Author.Username }}</a></b><span style="margin-left:5px"><i class="fa fa-sun-o"></i>{{ comment.ReplyTime | formatDate }}</span><br>{{ comment.Content }}</p></td> 
        </tr>
     </table>
    <p v-else>正在加载...</p>
